...
void Program()
{
    ...
    for(int i=0; i<n; i++)
    {
        ...
        if(generates)
        {
            GenerationAlgorithm(1000*pow(10,i),5);
        }
        ReadFile(arr,"kursiokai"+ConvertIntToString(1000*pow(10,i)));
        SortByName(arr);
        SortElements(arr,best,worst);
        PrintElements(best,worst);
        auto end=std::chrono::high_resolution_clock::now();
        std::chrono::duration<double> diff = end - start;
        cout << endl<<arr.size()<<" dydzio vektoriaus testavimas uztruko "<< diff.count() << " sekundziu\n";
        cout<<"-----------------------------\n";
        arr.clear();
        best.clear();
        worst.clear();
...
void SortByName(vector<Student> &arr)
{
    ...
    auto end = std::chrono::high_resolution_clock::now();
        std::chrono::duration<double> diff = end - start;
        cout << arr.size()<<" dydzio vektoriaus rusiavimas uztruko "<< diff.count() << " sekundziu\n";
}
...
void PrintElements(vector<Student> best, vector<Student> worst)
{
    ...
    ofstream fr2("nuskriaustieji"+ConvertIntToString(worst.size())+".txt");
    #pragma omp parallel for
